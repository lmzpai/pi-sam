...
from .data_config import collect_datasets
...
class LargeScaleJitter(object):
    ...
    def __init__(self, output_size=1024, aug_scale_min=0.8, aug_scale_max=1.6):
...
train_transforms = (RandomHFlip(), LargeScaleJitter())
test_transforms = (Resize(1024),)

def get_im_gt_name_dict(datasets, training, logger):
    if training:
        flag = 'train'
    else:
        flag = 'valid'
    ...
    for i in range(len(datasets)):
        ...
        logger.info(f"--->>> {flag} dataset {i}/{len(datasets)} {name},<<<---")
        tmp_im_list, tmp_gt_list = [], []
        tmp_im_list = glob(datasets[i]["im_dir"]+os.sep+'*'+datasets[i]["im_ext"])

        im_dir = datasets[i]["im_dir"]
        logger.info(f'-im- {name} {im_dir}: {len(tmp_im_list)}')

        gt_dir = datasets[i]["gt_dir"]
        if(gt_dir==""):
            logger.info(f'-gt- {name} {gt_dir}: No Ground Truth Found')
            tmp_gt_list = []
        else:
            tmp_gt_list = [datasets[i]["gt_dir"]+os.sep+x.split(os.sep)[-1].rsplit(datasets[i]["im_ext"],1)[0]+datasets[i]["gt_ext"] for x in tmp_im_list]
            logger.info(f'-gt- {name} {gt_dir}: {len(tmp_gt_list)}')
...
def create_dataloaders(ued_datasets, data_root, logger, batch_size=1, training=False):
    ...
    if training:
        for i in range(len(name_im_gt_list)):   
            gos_dataset = OnlineDataset([name_im_gt_list[i]], transform = transforms.Compose(train_transforms))
        ...
        if dist.is_dist_avail_and_initialized():
            ...
            dataloader = DataLoader(gos_dataset, batch_size=batch_size, num_workers=num_workers_, shuffle=True)
        ...
        for i in range(len(name_im_gt_list)):   
            gos_dataset = OnlineDataset([name_im_gt_list[i]], transform = transforms.Compose(test_transforms), eval_ori_resolution = True)
            dataset_names.append(name_im_gt_list[i]['dataset_name'])
            if dist.is_dist_avail_and_initialized():
    ...
    return gos_dataloaders, dataset_names
...
class OnlineDataset(Dataset):
    def __init__(self, name_im_gt_list, transform=None, eval_ori_resolution=False):
        ...
        for i in range(0,len(name_im_gt_list)):
            ...
            im_name_list.extend([x.split(os.sep)[-1].rsplit(name_im_gt_list[i]["im_ext"],1)[0] for x in name_im_gt_list[i]["im_path"]])
