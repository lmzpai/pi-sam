...
class MaskDecoderLargeFeat(MaskDecoder):
    ...
    def forward(self, encoder_outputs, interm_embeddings=None, labels=None, labels_256=None, get_loss=False):
        ...
        for b_i, enc_out in enumerate(encoder_outputs):
            enc_out['hq_feature'] = hq_features[b_i].unsqueeze(0)
            enc_out['rgb_embedding'] = interm_embeddings[-1][b_i]
    ...
    def predict_masks(
        ...
        upscaled_embedding = self.embedding_act(
            self.embedding_maskfeature(upscaled_embedding) + hq_feature)
