...
class MaskDecoderHQ(MaskDecoder):
    ...
    def forward(self, encoder_outputs, interm_embeddings,labels=None, labels_256=None, get_loss=False):
        batch_len = len(encoder_outputs)
        image_embeddings = torch.cat([encoder_outputs[b_i]['image_embeddings'] for b_i in range(batch_len)], dim=0)
        vit_features = interm_embeddings[0].permute(0, 3, 1, 2) # early-layer ViT feature, after 1st global attention block in ViT
        hq_features = self.embedding_encoder(image_embeddings) + self.compress_vit_feat(vit_features)
        pred_masks = []
        for bi, enc_out in enumerate(encoder_outputs):
            enc_out['hq_feature'] = hq_features[bi].unsqueeze(0)
            mask, _ = self.forward_sample(**enc_out)
